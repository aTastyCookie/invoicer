...
abstract class Calculator {
	...
	protected function calculateItems()
	{
		foreach ($this->items as $item)
		{
			...
			{
				$taxTotal = round($subtotal  * ($item['taxRatePercent'] / 100), 2);
			}
			else
			{
				$taxTotal = 0;
	...
	protected function calculateTaxRates()
	{
		foreach ($this->taxRates as $taxRate)
		{
			...
			{
				$taxTotal = round($this->calculatedAmount['item_subtotal'] * ($taxRate['taxRatePercent'] / 100), 2);

			}
			else
			{
				$taxTotal = round($this->calculatedAmount['total'] * ($taxRate['taxRatePercent'] / 100), 2);
			}
