...
use FI\Modules\Invoices\Mail\InvoiceMailer;
...
class InvoiceController extends \BaseController {
	...
	public function store()
	{
		...
		if (!$clientId)
		{
			$clientId = $client->create(array('name' => Input::get('client_name')))->id;
		}
		...
		$input = array(
			...
			'currency_code'     => $client->find($clientId)->currency_code,
			'exchange_rate'     => ''
		);
	...
	public function mailInvoice()
	{
		...
		$mailValidator = App::make('MailValidator')->getValidator(Input::all());

		if ($mailValidator->fails())
		{
			return Response::json(array(
				'success' => false,
				'errors'  => $mailValidator->messages()->toArray()
			), 400);
		}
