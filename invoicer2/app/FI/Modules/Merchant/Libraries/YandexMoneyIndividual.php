...
class YandexMoneyIndividual {

	public static function createGateway()
	{
		$gateway = Omnipay::create('YandexMoneyIndividual');

		$gateway->setAccount(Config::get('payments.merchants.YandexMoneyIndividual.account'));
		$gateway->setPassword(Config::get('payments.merchants.YandexMoneyIndividual.password'));
		$gateway->setCurrencyNum(Config::get('payments.merchants.YandexMoneyIndividual.currencyNum'));
	...
	public static function setPurchaseParameters($purchaseParameters, $params)
	{
		...
		if (isset($params['post']) && is_array($params['post'])) {
			...
			if (isset($params['post']['amount'])) {
				$purchaseParameters['amount'] = $params['post']['amount'];
			}
			...
			if (isset($params['post']['datetime'])) {
				$purchaseParameters['datetime'] = $params['post']['datetime'];
			}
			if (isset($params['post']['sender'])) {
				$purchaseParameters['sender'] = $params['post']['sender'];
			}
			if (isset($params['post']['codepro'])) {
				$purchaseParameters['codepro'] = $params['post']['codepro'];
			}
			if (isset($params['post']['currency'])) {
				$purchaseParameters['currency'] = $params['post']['currency'];
			}
			if (isset($params['post']['sha1_hash'])) {
				$purchaseParameters['sha1_hash'] = $params['post']['sha1_hash'];
			}	
			if (isset($params['post']['form_comment'])) {
				$purchaseParameters['form_comment'] = $params['post']['form_comment'];
			}
			if (isset($params['post']['comment'])) {
				$purchaseParameters['comment'] = $params['post']['comment'];
			}
