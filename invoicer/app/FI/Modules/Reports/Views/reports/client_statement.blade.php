...
<script src="{{ asset('js/FI/client_lookup.js') }}"></script>
	...
	$(function() {
		$('#btn-run-report').click(function() {
            ...
            $.post("{{ route('reports.clientStatement.ajax.validate') }}", {
                ...
                query_string = "?from_date=" + from_date + "&to_date=" + to_date + "&client_name=" + client_name;
                if (output_type == 'preview') {
                    $('#preview').show();
                    $('#preview-results').attr('src', "{{ route('reports.clientStatement.html') }}" + query_string);
                }
                else if (output_type == 'pdf') {
                    window.open("{{ route('reports.clientStatement.pdf') }}" + query_string);
                }
...
<aside class="right-side">
	...
	<section class="content-header">
		<h1>{{ trans('fi.client_statement') }}</h1>
	</section>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    {{ Form::text('client_name', null, array('id' => 'client_name', 'class' =>
